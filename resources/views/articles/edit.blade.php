<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Dashboard - Modification d\'article') }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-2xl w-full m-auto p-4 rounded-2xl">
        ...
        <div class="bg-secondary relative p-4 rounded-4xl">
            ...
            <form class="p-4" method="POST" action="{{ route('articles.update',$article->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div>
                ...
                <div class="mt-8">
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" value="{{$article->title}}" required autofocus />
                </div>

                <div class="mt-8">
                    <x-input-label for="description" :value="__('Description')" />
                    <textarea id="description" name="description" class="block w-full h-64 rounded-2xl">{{$article->description}}</textarea>
                </div>
