<x-app-layout>
    ...
    <div class="max-w-2xl w-full m-auto p-4 rounded-2xl">
        ...
        <div class="bg-secondary relative p-4 rounded-4xl">
            ...
            <form class="p-4" method="post" action="{{ route('pages.update',$page->id) }}">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" value="{{$page->title}}" required autofocus />
                </div>


                <div class="mt-8">
                    <x-input-label for="description" :value="__('Description')" />
                    <textarea class="block w-full h-64 rounded-2xl" id="description" name="description">{{$page->description}}</textarea>
                </div>


                <div class="flexflex-col mt-8">
                    <x-input-label for="order" :value="__('Ordre')" />
                    <select class="rounded-2xl" id="order" name="order" value="{{$page->order}}">
                        <?php $i=0?>
